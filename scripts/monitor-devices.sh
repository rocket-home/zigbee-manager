...
if [ -f ../.env ]; then
    # Загружаем только безопасные переменные (без запятых и специальных символов)
    while IFS= read -r line; do
        # Пропускаем комментарии и пустые строки
        if [[ ! "$line" =~ ^[[:space:]]*# ]] && [[ -n "$line" ]]; then
            # Проверяем, что строка содержит только безопасные символы для export
            if [[ "$line" =~ ^[A-Za-z_][A-Za-z0-9_]*= ]]; then
                            # Пропускаем системные переменные только для чтения
            var_name=$(echo "$line" | cut -d'=' -f1)
            if [[ "$var_name" != "UID" && "$var_name" != "GID" && "$var_name" != "SHELL" && "$var_name" != "PWD" ]]; then
                # Обрабатываем переменные в кавычках
                if [[ "$line" =~ ^[A-Za-z_][A-Za-z0-9_]*=\" ]]; then
                    # Переменная в кавычках - экспортируем как есть
                    export "$line" 2>/dev/null || true
                elif [[ ! "$line" =~ , ]]; then
                    # Переменная без запятых - экспортируем
                    export "$line" 2>/dev/null || true
                fi
            fi
            fi
