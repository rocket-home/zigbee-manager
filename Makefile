...
setup: ## Первоначальная настройка системы (без параметров безопасности)
	...
	@$(MAKE) permissions
	@echo "$(YELLOW)2️⃣ Проверка группы dialout...$(NC)"
	@$(MAKE) apply-group
...
apply-group: ## Применить группу dialout без перезагрузки
	...
	@if groups $$USER | grep -q dialout; then \
		echo "$(GREEN)✅ Пользователь $$USER уже в группе dialout$(NC)"; \
		echo "$(YELLOW)💡 Группа dialout уже применена в текущей сессии$(NC)"; \
	else \
		echo "$(RED)❌ Пользователь $$USER не в группе dialout$(NC)"; \
		echo "$(YELLOW)💡 Сначала выполните: make permissions$(NC)"; \
		echo "$(YELLOW)💡 Затем перезайдите в систему или выполните 'newgrp dialout' вручную$(NC)"; \
	fi
...
clean: ## Очистить все данные (ОСТОРОЖНО!)
	@if [ -z "$$FORCE" ]; then \
		...
		echo "$(YELLOW)💡 Конфигурации, данные MQTT и Zigbee2MQTT, а также логи будут потеряны.$(NC)"; \
	...
	@sudo rm -rf mqtt/data/* mqtt/log/* zigbee2mqtt/data/*
