...
env: ## Создать .env из примера и открыть в редакторе
	...
	@echo "$(BLUE)📝 Открытие .env в редакторе...$(NC)"
	@if command -v code >/dev/null 2>&1; then \
		echo "$(GREEN)✅ Открываю в Visual Studio Code...$(NC)"; \
		code $(ENV_FILE); \
	elif command -v nano >/dev/null 2>&1; then \
		echo "$(GREEN)✅ Открываю в nano...$(NC)"; \
		nano $(ENV_FILE); \
	elif command -v vim >/dev/null 2>&1; then \
		echo "$(GREEN)✅ Открываю в vim...$(NC)"; \
		vim $(ENV_FILE); \
	elif command -v vi >/dev/null 2>&1; then \
		echo "$(GREEN)✅ Открываю в vi...$(NC)"; \
		vi $(ENV_FILE); \
	else \
		echo "$(RED)❌ Не найден подходящий редактор$(NC)"; \
		echo "$(YELLOW)💡 Установите один из редакторов: code, nano, vim, vi$(NC)"; \
		echo "$(BLUE)📝 Файл .env создан: $(ENV_FILE)$(NC)"; \
	fi
